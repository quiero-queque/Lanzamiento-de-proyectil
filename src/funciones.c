...
float redondear(float valor) {
    float redondeado= roundf(valor * 100) / 100.0f;
    return redondeado;
}
float calcular_radianes(float angulo){
    float radianes= (angulo*PI)/180;
    return redondear(radianes);
}
